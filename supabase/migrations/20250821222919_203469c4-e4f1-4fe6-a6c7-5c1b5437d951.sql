-- Fix rate_limits table constraint and function
-- First add unique constraint for the ON CONFLICT to work
ALTER TABLE rate_limits ADD CONSTRAINT rate_limits_player_action_unique UNIQUE (player_id, action_type);

-- Fix the rate limiting function to work properly
...
BEGIN
  ...
  -- Record this action with proper upsert
